...
5. Le navigateur revient au callback exact de l'hôte. L'hôte échange le code et stocke sa session MCP. L'utilisateur ne copie jamais de code d'autorisation et ne voit ni ne colle de jeton.

L'hôte MCP reçoit uniquement un jeton opaque destiné à `https://lcamcp.tiangong.earth/mcp`. Le service conserve une session Supabase distincte et chiffrée pour Edge Functions ou PostgREST ; le jeton MCP entrant n'est jamais transmis en aval.

Ne donnez jamais à une IA, un chat, un argument de commande ou un en-tête manuel un nom d'utilisateur, mot de passe, code d'autorisation, jeton d'accès/rafraîchissement ou l'ancienne clé API utilisateur. Le service distant n'expose plus de page de démonstration/affichage de code OAuth et ne prend en charge ni grant password ni client-credentials.
...
3. Utilisez l'identifiant client préenregistré fourni par l'opérateur. Ne générez pas de client et ne collez ni clé API ni jeton Bearer.
...
- Une déconnexion locale doit supprimer la session MCP de l'hôte, mais ne remplace pas la révocation côté compte.
- Révoquer « TianGong LCA MCP » dans les **applications connectées** bloque immédiatement les nouveaux rafraîchissements Supabase. Un jeton d'accès court déjà émis peut rester valide jusqu'à son expiration ; les opérations sensibles appliquent leur propre contrat de vérification en ligne.
