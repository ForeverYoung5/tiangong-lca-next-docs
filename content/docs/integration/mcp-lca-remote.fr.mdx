...
Avant la connexion, ajoutez l'identifiant public et le callback loopback exact enregistré dans `~/.codex/config.toml` :
...
`callback_url` doit correspondre octet pour octet à l'URI enregistrée, y compris le schéma, l'hôte, le port, le chemin et le suffixe fixe. Codex ouvre le navigateur et conserve le refresh token rotatif localement. N'activez pas DCR, n'ajoutez pas de secret client et ne collez pas de jeton Bearer.
