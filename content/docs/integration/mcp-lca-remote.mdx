...
Codex 会把这个 base 与该 MCP URL 的稳定 callback ID 组合成 `http://127.0.0.1:49193/callback/sB-dwg9ebTQE`；这是 Supabase 中逐字登记的最终 redirect URI。显式 URL 端口不会自动设置监听器，因此 `mcp_oauth_callback_port` 也必须保持为 `49193`。Codex 会打开浏览器，并把 rotating refresh token 保存在本地凭据存储中。不要开启 DCR，不要添加 client secret，也不要粘贴 bearer token。
