...
使用运维为当前 Codex host 登记的 public client ID 与精确 loopback callback：
...
Codex 会打开浏览器，并把 rotating refresh token 保存在本地凭据存储中。如果 loopback callback 与登记值不同，应停止并让运维登记精确的 client/callback pair；不要开启 DCR，也不要粘贴 bearer token。
