...
远程命令使用 Supabase OAuth 2.1 浏览器授权。配置 API 地址、publishable key 和环境专属的 public OAuth client ID：
...
- `TIANGONG_LCA_OAUTH_CLIENT_ID` 是公开 client ID，不是密钥；Dev 与 Production 的 ID 不同。
...
不要向 AI、脚本参数或聊天窗口提供用户名、密码、authorization code、access token、refresh token 或旧的可逆用户 API Key。批准的 headless 自动化只能由 orchestrator 注入一个短期 `TIANGONG_LCA_ACCESS_TOKEN`；Supabase OAuth 不提供 password 或 client-credentials grant。多账号工作应为每个 account/project/client 使用独立私有 `TIANGONG_LCA_SESSION_FILE`。
...
`tiangong-lca dataset maintenance plan/apply/verify` 面向受控的账号级数据清理、坏导入修复和
支持数据别名合并。它以当前认证用户运行，依赖服务端 RLS 限制可见行和可写行；不要把它当成
跨账号管理员批处理工具。
