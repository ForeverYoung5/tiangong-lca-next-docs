...
Never give an AI, command argument, or chat window a username, password, authorization code, access token, or refresh token. Approved headless automation may receive one short-lived `TIANGONG_LCA_ACCESS_TOKEN` from its orchestrator; Supabase OAuth provides neither password nor client-credentials grants. Use a separate private `TIANGONG_LCA_SESSION_FILE` for each account/project/client context.
