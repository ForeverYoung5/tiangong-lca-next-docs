...
Remote-Befehle verwenden Supabase OAuth 2.1 mit Browser-Autorisierung. Konfigurieren Sie API-URL, Publishable Key und die umgebungsspezifische öffentliche OAuth-Client-ID:
...
- `TIANGONG_LCA_OAUTH_CLIENT_ID` ist eine öffentliche Kennung, kein Geheimnis; Dev und Production verwenden unterschiedliche IDs.
...
Geben Sie KI, Kommandoargumenten oder Chats niemals Benutzername, Passwort, Autorisierungscode, Access-/Refresh-Token oder den ausgemusterten reversiblen Nutzer-API-Key. Genehmigte Headless-Automatisierung darf nur ein kurzlebiges `TIANGONG_LCA_ACCESS_TOKEN` vom Orchestrator erhalten; Supabase OAuth bietet weder Password- noch Client-Credentials-Grant. Verwenden Sie je Account/Projekt/Client eine separate private `TIANGONG_LCA_SESSION_FILE`.
...
der Reparatur fehlerhafter Importe und dem Zusammenführen von Support-Daten-Aliasen. Es läuft als
aktuell authentifizierter Nutzer und stützt sich auf serverseitige RLS, um sichtbare und schreibbare
Zeilen zu begrenzen; behandeln Sie es nicht als kontoübergreifendes Admin-Batch-Werkzeug.
