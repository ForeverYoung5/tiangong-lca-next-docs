...
Use the public client ID and exact loopback callback registered by the operator for this Codex host:
...
Codex opens the browser and keeps the rotating refresh token in its local credential store. If its loopback callback differs from the registered value, stop and have the operator register the exact client/callback pair; do not enable DCR or paste a bearer token.
