...
Before login, put the public client ID and its exact operator-registered loopback callback in `~/.codex/config.toml`:
...
`callback_url` must match the registered URI byte-for-byte, including scheme, host, port, path, and any fixed callback suffix. Codex opens the browser and keeps the rotating refresh token in its local credential store. Do not enable DCR, add a client secret, or paste a bearer token.
