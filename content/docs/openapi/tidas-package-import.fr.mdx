...
description: "Importer des packages TIDAS ZIP via l'API TianGong LCA avec authentification par clé API."
---
...
- Les sessions navigateur internes peuvent toujours utiliser `USER_JWT`, mais les intégrations
  API externes devraient privilégier `USER_API_KEY`
...
[`tidas` 0.1.3 CLI](/fr/docs/integration/cli/#installer-tidas-013) before calling the
...
- `prepare_upload` seul ne suffit pas ; téléversement, mise en file et interrogation sont
  tous requis
- L'authentification par clé API et le JWT navigateur partagent le même contrat d'endpoint ; les
  intégrations externes devraient préférer les clés API
