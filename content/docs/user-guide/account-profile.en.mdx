...
- After revocation, the client must start browser authorization again. Run `tiangong-lca auth login` for the CLI or reconnect the MCP host.
...
The reversible user API key is a retired compatibility path, not a setup option for new integrations. Never give an AI or external tool a password, authorization code, access token, refresh token, or legacy API key.
