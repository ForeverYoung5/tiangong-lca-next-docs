...
- Après révocation, le client doit recommencer OAuth dans le navigateur : `tiangong-lca auth login` pour la CLI ou reconnexion de l'hôte MCP.
...
L'ancienne clé API utilisateur réversible est un chemin de compatibilité retiré, pas une configuration pour les nouvelles intégrations. Ne donnez jamais à une IA ou un outil externe un mot de passe, code d'autorisation, jeton d'accès/rafraîchissement ou ancienne clé API.
