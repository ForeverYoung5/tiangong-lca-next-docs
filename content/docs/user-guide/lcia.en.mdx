...
- If the process has a current Model / Result Release, the process viewer can also show release
  version, readback status, validation items, manifest hashes, and downloadable ZIPs. Legacy
  LCIA-only results remain separate; they are not automatically the same as a Calculation Bundle or
