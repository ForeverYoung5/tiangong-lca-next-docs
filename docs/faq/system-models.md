...
## 没有显式 market process 时，地区供应 mix 如何处理？

当一个产品输入交换没有直接指向单一 provider 时，TianGong LCA 会优先根据交换项 Location
确定供给地区；如果交换项没有 Location，则使用消费过程自身的地区作为默认供给地区。

在同一供给地区内，如果有多个 provider 过程可供应同一参考流，系统会使用这些 provider 的年度
供应或产量作为相对权重来分摊该输入需求。这个年度量只表示 provider share，不会改变消费过程
原本的输入需求总量。

构建可计算快照时，coverage 诊断会记录 provider 匹配策略、地理层级、供给地区来源、年度量
权重可用性以及 no-provider gap。若区域供应 mix 与预期不一致，请优先核对交换项 Location、
消费过程地区和 provider 年度供应或产量。
