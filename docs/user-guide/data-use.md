...
在过程、流、来源、单位组、流属性、联系人和模型等数据列表中，可以使用搜索区的**引用查找**模式反查哪些数据包含某个数据集 UUID。

1. 在列表搜索区勾选**引用查找**，系统会自动关闭智能推荐模式。
2. 输入完整的数据集 UUID。未输入完整 UUID 时，页面会提示“请输入完整的数据集 UUID 后再进行引用查找”。
3. 点击**查找包含该 ID 的数据**或按回车执行查找。
4. 结果会继续遵循当前数据空间、团队、状态和数据类型筛选；如命中较多，页面最多展示前 50 条引用查找结果。

当您需要评估删除、改版或替换某个数据集前的影响范围时，优先使用引用查找确认哪些数据仍在引用该 ID。
