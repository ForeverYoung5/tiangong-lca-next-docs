...
3. **提交审核**：确认无误后点击 `Submit for review`。过程数据提交时，平台会保存当前版本、重新执行
   `Data check`，并运行**数值稳定性门禁**；只有门禁通过后才会正式提交审核。若门禁显示排队、运行中、
   已阻止、已过期或执行失败，请按提示修复数据、保存最新版本并重新提交。
4. **关注反馈**：提交后关注通知中心或[数据审核](./data-review)中的状态筛选了解进度。
5. **团队协作**：若需他人协助，可通过“贡献给团队”将数据同步至团队空间，再结合[团队功能](./team-function)进行协作。
