...
平台支持自定义单位组与流属性，但自定义配置会影响后续 LCIA 计算，请谨慎评估再使用。当前“我的数据”中的单位组和流属性列表可供普通用户搜索、查看、查看所有版本和导出，便于核对已有基础配置；新增、导入、复制、新增版本、编辑、删除和贡献等维护入口仅系统管理员可用。如果页面提示需要新增或补充，请联系管理员。
...
2. **自检**：提交审核前运行 `Data check`，平台会自动检测必填项与引用完整性。当前表单会尽量把 SDK 校验问题定位到对应标签和字段；引用数据的 ID 或版本、分类字段、过程交换里的多语言备注、功能单位说明，以及字符长度超限等问题，会在相关标签或字段旁显示修复提示，例如当前字符数和允许上限。  
