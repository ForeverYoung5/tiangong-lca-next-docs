...
1. 点击“创建团队”。  
2. 填写团队名称、简介等基本信息；“公开团队信息”默认关闭，仅在需要他人通过邮箱联系团队负责人时开启。  
3. 创建完成后您将自动成为团队所有者。  
...
1. **发送邀请**：所有者/管理员在团队成员页面输入已注册用户的邮箱并发送邀请。  
   ![发送团队邀请](img/add-member.png) ![邀请确认邮件](img/team-member-email.png)
2. **接收通知**：被邀请者在右上角通知中心收到提醒，确认后加入团队。  
...
- **操作按钮说明**：请参考[按键功能介绍](./key-functions-introduction#协作动作)了解邀请、移除等按钮的位置与作用。
...
- **邀请对象未收到通知？**  
  提醒对方检查右上角通知中心；若仍未收到，请确认邮箱对应的账号已经注册、尚未加入其他团队且没有其他团队的待处理邀请。若该用户曾拒绝本团队邀请，请在成员列表中使用“重新邀请”。
- **团队数据无法编辑？**  
  团队空间以共享为主，只读模式避免多人冲突。请复制到个人“我的数据”中进行修改，再贡献回团队。

如需同时对照团队角色、审核角色、系统角色和数据空间之间的差异，请参考
